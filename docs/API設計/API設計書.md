...
**再生成の制限（要件定義書 F-008 準拠）：** 成功した返信を作り直す再生成機能は設けない。`content` 省略によるAIフィードバック生成要求は、対象の家計簿レコードに `completed` 状態のAI投稿が既に存在する場合は 409 で拒否する。`failed` 状態の場合のみ再試行を許可する。

`content` 省略 + 既に completed のAI投稿がある場合のエラー（409）:

```json
{
  "message": "AIフィードバックは既に生成済みです",
  "errors": {}
}
```
