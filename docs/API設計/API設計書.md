...
**重複制御：** 1つの家計簿レコードに登録できる自己レビューは1件まで。既に自己レビューが存在する場合は 409 を返す。内容を変更する場合は編集（F-014）を使用する。

既に自己レビューが登録済み（409）:

```json
{
  "message": "この家計簿レコードには既に自己レビューが登録されています",
  "errors": {}
}
```

**`self_reviews` と `posts` の関係：**

- `self_reviews` はユーザーが家計簿に対して行う振り返りの記録であり、AIとの会話（`posts`）とは独立している
- `posts` はAIとの会話スレッド。家計簿情報と自己レビュー（存在する場合）をもとに AI がフィードバックを生成して開始する。自己レビューがなくても家計簿情報のみでスレッドを開始できる（F-008）
- スレッド内のやり取りは `posts` テーブルで完結する。`is_ai` でユーザー投稿とAI投稿を区別し、`parent_id`（自己参照FK）でリプライチェーンを構成する
- S-005 のスレッド表示では、自己レビュー・AI返信・以降のやり取りを同じ領域に表示する（要件定義書 S-005）。フロントエンドは `/api/v1/records/{recordId}/reviews` と `/api/v1/records/{recordId}/posts` を両方取得し、`createdAt` で時系列に組み合わせて表示する
- 自己レビューの編集（F-014）は `self_reviews` のみ更新する。`posts` のAI返信は再生成しない（要件定義書 F-014 準拠）。フロントは最新の `self_reviews` を表示するため、スレッド上の表示も自動的に反映される
- 自己レビューの削除（F-015）は `self_reviews` のみ論理削除する。紐づくAI返信やその後のやり取り（`posts`）は削除しない。スレッド上から自己レビューが消え、AI返信以降のやり取りは残る
...
**再生成・重複の制限（要件定義書 F-008 準拠）：** 成功した返信を作り直す再生成機能は設けない。`content` 省略によるAIフィードバック生成要求は、対象の家計簿レコードに `pending` / `processing` / `completed` 状態のAI投稿が既に存在する場合は 409 で拒否する。`failed` 状態の場合のみ再試行を許可する。

`content` 省略 + 既に `pending` / `processing` / `completed` のAI投稿がある場合のエラー（409）:

```json
{
  "message": "AIフィードバックは既に生成中または生成済みです",
  "errors": {}
}
```

**追加チャットの送信制限：** `content` 指定時（追加チャット）であっても、対象の家計簿レコードに `pending` / `processing` 状態のAI投稿が存在する場合は 409 で拒否する。AI返信が確定する前に次の投稿を送信すると、会話順序や親子関係が不安定になるため。

`content` 指定 + `pending` / `processing` のAI投稿がある場合のエラー（409）:

```json
{
  "message": "AIの返信を待っている間は投稿できません",
  "errors": {}
}
```
